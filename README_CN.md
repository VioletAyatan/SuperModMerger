...
- **多类型支持**：支持多种使用scr结构的脚本合并，以及.xml、.gui等文件合并，支持逐行合并与冲突检测功能
- **合并逻辑**：默认的合并逻辑基于原版文件进行合并，这可能会修复一些过期MOD的问题，但仅限多个MOD之间包含相同文件时才会如此；因为不重复的文件会直接复制，不会进行解析。你也可以在程序运行时通过交互选择“全局修复模式”，这会对合并的所有文件进行解析，有可能修复一些过期mod的问题，但会花费更多时间。
...
- `-m`, `--merge <path>`：指定待合并 mods 目录，默认是当前工作目录下的 `mods`。
- `-o`, `--output <path>`：指定输出 pak 文件路径，默认是当前工作目录下的 `source/data7.pak`。
- `-b`, `--base <path>`：指定基准 pak 文件路径，默认是当前工作目录下的 `source/data0.pak`。
- `-h`, `--help`：显示帮助信息。

### 实际运行行为

- 程序会递归扫描 mods 目录，支持 `.pak`、`.zip`、`.7z` 格式。
- “全局修复模式”当前是运行时交互选择，不是命令行参数。
- 对于无法智能合并、但相对路径冲突的二进制资源，程序会提示用户选择保留哪个版本。
